...
public class ProjectService {
    ...
    public ProjectListResponseDTO findProjects(long departmentId, int pageNumber, int pageSize)
    ...
    public ProjectsWithEmployeesListDTO findProjectsWithEmployeesSalariesHours(int pageNumber, int pageSize,
            long numberOfEmployees,
            long totalHours, BigDecimal totalSalaries) {
        int offset = (pageNumber <= 1 ? 0 : pageNumber - 1) * pageSize;
        List<ProjectsWithEmployeesDTO> list = projectDAO.findProjectsWithEmployeesSalariesHours(offset, pageSize,
                numberOfEmployees, totalHours, totalSalaries);
        ProjectCountDTO projectCountDTO = projectDAO.findTotalCountProjectsWithEmployeesSalariesHours(numberOfEmployees,
                totalHours, totalSalaries);
    ...
    public ByteArrayOutputStream exportExcelProjectsWithEmployeesSalariesHours(long numberOfEmployees, long totalHours,
            BigDecimal totalSalaries, String projectIdsParam) throws Exception {
        ...
        List<ProjectsWithEmployeesDTO> list = projectDAO.findProjectsWithEmployeesSalariesHours(numberOfEmployees,
                totalHours, totalSalaries, projectIds, arr[0]);
        Workbook workbook = new XSSFWorkbook();
        Sheet sheet = workbook.createSheet("Project with salaries");
        setWidthColumn(sheet);

        List<String> titles = new ArrayList<>(
                List.of("No.", "Project's name", "Area", "Number of employees", "Total hours", "Total salaries"));
    ...
    private CellStyle createCellStyle(Sheet sheet, HorizontalAlignment alignment, int cellColor, int fontColor,
            boolean bold) {
        // Create font
    ...
    private void createHeader(Sheet sheet, List<String> titles) {
        ...
        for (int i = 0; i < titles.size(); i++) {
            ...
            row.getCell(i).setCellStyle(createCellStyle(sheet, HorizontalAlignment.CENTER,
                    IndexedColors.LIGHT_BLUE.getIndex(), IndexedColors.WHITE.getIndex(), true));
        }
    }
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            if (i % 2 == 0) {
                for (int j = 0; j < titles.size(); j++) {
                    ...
                    row1.getCell(j).setCellStyle(createCellStyle(sheet, alignment,
                            IndexedColors.LIGHT_TURQUOISE1.getIndex(), IndexedColors.BLACK.getIndex(), false));
                }
            } else {
                for (int j = 0; j < titles.size(); j++) {
                    HorizontalAlignment alignment = HorizontalAlignment.LEFT;
                    ...
                    row1.getCell(j).setCellStyle(createCellStyle(sheet, alignment, IndexedColors.WHITE.getIndex(),
